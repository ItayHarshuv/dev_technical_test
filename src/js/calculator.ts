...
class Calculator {
  ...
  private attachDataDrivenValidators(
    suffix: string,
    ...
    fields.forEach((field) => {
      if (field.tagName === "INPUT") {
        field.addEventListener("blur", () =>
          this.validateField(field as HTMLInputElement, form),
        );
        ...
        field.addEventListener("change", () =>
          this.validateField(field as HTMLSelectElement, form),
        );
        field.addEventListener("change", () =>
          this.clearFieldError(field as HTMLSelectElement),
        );
      }
    });
  }
