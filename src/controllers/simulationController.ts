    ...
    try {
        ...
        if (!propertyPurchasePrice || !monthlyRentalAmount || !annualRentalFee || !prospectEmailAddress) {
            res.status(400).json({ 
                ...
                message: 'All fields (propertyPurchasePrice, monthlyRentalAmount, annualRentalFee, prospectEmailAddress) are required'
            });
            return;
        }
        ...
        if (propertyPurchasePrice <= 0 || monthlyRentalAmount <= 0 || annualRentalFee <= 0) {
            res.status(400).json({ 
                ...
                message: 'Purchase price, monthly rent, and annual fee must be greater than zero'
            });
            return;
        }
